...
document.getElementById('login-form').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                window.location.href = '/api/View/pageBank';
            }, 1500);
        } else {
            // Ошибка входа
...
document.getElementById('register-form').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('https://localhost:44319/api/Auth/register', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (response.ok) {
            ...
            // Ошибка регистрации
            showMessage(messageDiv, data.message || `Ошибка регистрации: ${errorDetails.replace(/\n\s+/g, '\n')}`, 'error');
        }
    } catch (error) {
        showMessage(messageDiv, `Ошибка cети: ${errorDetails.replace(/\n\s+/g, '\n')}`, 'error');
